...
def process_mocap(body_data_dict, paths, settings, is_augment):
    ...
    trans = copy2cuda(body_data_dict['trans'][:])               # global translation
    ...
    body_betas = copy2cuda(body_data_dict['betas'][:BODY_NUM_BETAS, None])    # body shape parameters
...
def process_seq(data_paths, exception_file, eligible_mocap_file, settings):
    ...
    # load in input data (AMASS)
    try:
        bdata = np.load(input_file_path, allow_pickle=True)
        gender = np.array(bdata['gender'], ndmin=1)[0]
        gender = str(gender, 'utf-8') if isinstance(gender, bytes) else str(gender)
    # ARCTIC 
    except:
    ...
    mocap_data_dict = {}
    data_names = ['mocap_frame_rate', 'poses', 'trans', 'betas']
    ...
    try:
        ...
        pose_left_hand = bdata['poses'][:, 75:120]  # finger articulation joint rotations
        pose_right_hand = bdata['poses'][:, 120:]  # finger articulation joint rotations
    ...
    if left_var_flag:
        ...
        mocap_data_dict['poses'], mocap_data_dict['trans'] = reflect_body(b_pose=mocap_data_dict['poses'], root_transl=mocap_data_dict["trans"], root_rot=True)
...
def main(config):
    ...
    for datasetname in config.amass_datasets:
        os.system(f"rm -rf ./data/amass_processed_hist/{datasetname}")
    ...
    if os.path.isdir(config.amass_root):
        all_dataset_dirs += [os.path.join(config.amass_root, f) for f in sorted(os.listdir(config.amass_root)) if
                        f[0] != '.']
        dataset_dirs += [os.path.join(config.amass_root, f) for f in config.amass_datasets]
        dataset_names += config.amass_datasets
    ...
    for data_dir, data_name in zip(dataset_dirs, dataset_names):        
        ...
        input_seqs = glob.glob(os.path.join(data_dir, '*/*.npz'))
...
if __name__ == "__main__":
    ...
    parser.add_argument('--amass_root', type=str, default="./data/amass_raw", help='Root directory of raw AMASS dataset.')
