...
from utils.logger_utils import mkdir, Logger
from utils.body_utils import mano_full_chain 
from utils.transform_utils import rotation_matrix_to_angle_axis, batch_rodrigues, convert_to_rotmat
...
def read_keypoints(keypoint_fn):
    ...
    rh_keypoints =  np.array(person_data['hand_right_keypoints_2d'], dtype=np.float)
    rh_keypoints =  rh_keypoints.reshape([-1, 3])
    
    lh_keypoints =  np.array(person_data['hand_left_keypoints_2d'], dtype=np.float)
    lh_keypoints =  lh_keypoints.reshape([-1, 3])
        
    return rh_keypoints, lh_keypoints


def read_cam_extrinsics(keypoint_fn):
    '''
    Reads camera extrinsics of a timeframe.
    '''
    with open(keypoint_fn) as keypoint_file:
        data = json.load(keypoint_file)

    try:
        cam_t = np.array(data['camera_position'], dtype=np.float)
        cam_R = np.array(data['camera_rotation'], dtype=np.float)
    except:
        print('WARNING: Found no camera extrinsics in %s! Returning zeros!' % (keypoint_fn))
        cam_t = np.zeros((3), dtype=np.float)
        cam_R = np.eye((3), dtype=np.float)

    return np.concatenate((cam_R, cam_t.reshape(3,1)), axis=1)
...
def save_rgb_stitched_result(seq_intervals, all_res_out_paths, res_out_path, device, hand_model_path, body_model_path, num_betas, use_joints2d):
    seq_overlaps = [0]
    ...
    for res_idx, res_dir in enumerate(all_res_dirs):
        ...
        cur_stage3_res = prep_mano_result(cur_stage3_res, device, cur_stage3_res['trans'].shape[0])
        ...
        if concat_joints2d is None:
            concat_joints2d = cur_obs['rh_joints2d']
        else:
            concat_joints2d = np.concatenate([concat_joints2d, cur_obs['rh_joints2d'][seq_overlaps[res_idx]:]], axis=0)
    ...
    res_trans = concat_cam_res['transl'].clone().detach().cpu().numpy()
    res_root_orient = concat_cam_res['global_orient'].clone().detach().cpu().numpy()
    res_body_pose = concat_cam_res['hand_pose'].clone().detach().cpu().numpy()
    ...
    # get body model
    num_viz_frames = concat_cam_res['transl'].size(0)
    viz_body_model = BodyModel(model_path=hand_model_path, model_type="mano", device=device, num_betas=10, batch_size=num_viz_frames,
                               name="right_hand_optimized", skeleton_flag = False, **{"is_rhand":True})

    # concat_cam_res["transl"] = concat_cam_res.pop("trans")
    # concat_cam_res["global_orient"] = concat_cam_res.pop("root_orient")
    # concat_cam_res["hand_pose"] = concat_cam_res.pop("pose_body")


    viz_body = run_mano(concat_cam_res, viz_body_model)
    

    # transform full camera-frame sequence into a shared prior frame based on a single ground plane
    viz_joints3d = viz_body.joints
    # compute the transformation based on t=0 and the first sequence floor plane


    cam2prior_R, cam2prior_t, cam2prior_root_height = torch.eye(3).to(viz_joints3d).unsqueeze(0), torch.zeros(3).to(viz_joints3d).unsqueeze(0), torch.zeros(1).to(viz_joints3d)

    # cam2prior_R, cam2prior_t, cam2prior_root_height = compute_cam2prior(concat_ground_planes[0].unsqueeze(0),
    #                                                                     concat_cam_res['transl'][0].unsqueeze(0),
    #                                                                     concat_cam_res['global_orient'][0].unsqueeze(0),
    #                                                                     viz_joints3d[0].unsqueeze(0))
    # # transform the whole sequence
    input_data_dict = {kb : vb.unsqueeze(0) for kb, vb in concat_cam_res.items() if kb in ['transl', 'global_orient', 'hand_pose', 'betas']}
    viz_prior_data_dict = apply_cam2prior(input_data_dict, cam2prior_R, cam2prior_t, cam2prior_root_height, 
                                            input_data_dict['hand_pose'],
                                            input_data_dict['betas'],
                                            0,
                                            viz_body_model)
    
    concat_prior_res = {
        'transl' : viz_prior_data_dict['transl'][0],
        'global_orient' : viz_prior_data_dict['global_orient'][0],
        'hand_pose' : concat_cam_res['hand_pose'],
    ...
    res_trans = concat_prior_res['transl'].clone().detach().cpu().numpy()
    res_root_orient = concat_prior_res['global_orient'].clone().detach().cpu().numpy()
    res_body_pose = concat_prior_res['hand_pose'].clone().detach().cpu().numpy()
    # res_contacts = concat_contacts.clone().detach().cpu().numpy()
    res_contacts = None
    # np.savez(concat_prior_res_out_path, betas=res_betas, trans=res_trans, root_orient=res_root_orient, pose_body=res_body_pose, contacts=res_contacts)
...
def prep_mano_result(np_res, device, T):
    '''
    Load np result dict into dict of torch objects for use with MANO body model.
    '''
    betas = np_res['betas']
...
def run_mano(res_dict, hand_model):
    
    # only include the arguments that the model expects    
    input_dict = {k: v for k, v in res_dict.items() if k in hand_model.hand_argument_set}

    rh_out = hand_model(input_dict=input_dict)    
    return rh_out
...
def apply_cam2prior(data_dict, R, t, root_height, body_pose, betas, key_frame_idx, body_model, inverse=False):
    ...
    '''
    prior_dict = dict()
    if 'global_orient' in data_dict:
        # B x T x 3
        root_orient = data_dict['global_orient']
        ...
        prior_dict['global_orient'] = prior_root_orient

    if 'transl' in data_dict and 'global_orient' in data_dict:
        # B x T x 3
        trans = data_dict['transl']
        B, T, _ = trans.size()
        R_time = R.unsqueeze(1).expand((B, T, 3, 3))
        t_time = t.unsqueeze(1).expand((B, T, 3))
        if inverse:
            ...
            cur_mano_out = body_model(input_dict={ "hand_pose":body_pose.reshape((-1, body_pose.size(2))), 
                                                    "betas":betas.reshape((-1, betas.size(2))),
                                                    "global_orient":prior_dict['global_orient'].reshape((-1, 3)),
                                                    "transl":trans.reshape((-1, 3))})
            smpl_joints3d = cur_mano_out.joints.reshape((B, T, -1, 3))
            if T > 1:
...
OP_NUM_JOINTS = 16
OP_IGNORE_JOINTS = []                               # include all joints
OP_EDGE_LIST = mano_full_chain(is_rhand=True)
...
def expid2model(expr_dir):
    ...
    # best_model_fname = sorted(glob.glob(os.path.join(expr_dir, '*.pt')), key=os.path.getmtime)[-1]
    # try_num = os.path.basename(best_model_fname).split('_')[0]

    best_model_right_fname = os.path.join(expr_dir, 'hposer_right.pt')
    best_model_left_fname = os.path.join(expr_dir, 'hposer_left.pt')
    ...
    return ps, best_model_right_fname, best_model_left_fname

def load_hposer(expr_dir, vp_model='snapshot'):
    '''
    :param expr_dir:
    ...
    ps, trained_model_right_fname, trained_model_left_fname = expid2model(expr_dir)
    ...
    return hposer_right_pt, hposer_left_pt, ps