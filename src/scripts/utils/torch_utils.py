...
def cuda_cast(iterable):
    if type(iterable) == torch.Tensor:
        return iterable.to(dev)
    elif type(iterable) == np.ndarray:
        return torch.tensor(iterable, dtype=torch.float32).to(dev)
    else: 
        print("Exception")
        raise Exception("Not compatible input type")

copy2cpu = lambda tensor: tensor.detach().cpu().numpy()
copy2cuda = lambda iter: cuda_cast(iter)   
