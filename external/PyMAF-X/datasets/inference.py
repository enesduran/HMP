...
class Inference(Dataset):
    ...
    full_body=False, person_ids=[], wb_kps={}):
        self.pre_load_imgs = pre_load_imgs
        
        if pre_load_imgs is None:
            
            if "DexYCB" in image_folder:
                self.image_file_names = [osp.join(image_folder, x) for x
                        in os.listdir(image_folder) if x.endswith('.jpg')]
            else:
                self.image_file_names = [osp.join(image_folder, x) for x in os.listdir(image_folder) 
        ...
        self.raw_bboxes = wb_kps["bbox"]
        self.handedness = wb_kps["handedness"]
        self.bboxes = bboxes
        ...
        if self.has_keypoints:
            ...
            if not self.full_body:
                ...
                self.bboxes = joints2d[time_pt1:time_pt2][ :4]
            ...
            # enters here self.full_body = True    
            else:
                bboxes = []
                scales = []
            ...
            # enters here
                ...
                for part in ['lhand', 'rhand', 'face']:
                    ...
                    if len(kp2d_valid) > 0:
                        ...
                        ###################################################################### use the gt bounding box 
                        
                        if part == 'rhand':
                            # we may need different configurations for dexycb and ho3d 
                            img_name = self.image_file_names[idx]
                                                 
                            # we could have direcly used self.bboxes instead of loading the pickle file again BUT we dont because the bbox
                            # sizes are rescaled and therefore cropping method self.rgb_processing should not be called in that case. 
                            if "HO3D" in img_name:
                                # drop ./rgb/xxxx.jpg ./mmpose_keypoints2d/xxxx.pkl
                                pickle_name = os.path.join(os.path.dirname(os.path.dirname(img_name)), "mmpose_keypoints2d", os.path.basename(img_name).replace("jpg", "pkl"))    
                             
                            elif "DexYCB" in img_name:
                                index = int(img_name.split("_")[-1][:-4]) # read index from the image name
                                pickle_name = os.path.join(os.path.dirname(os.path.dirname(img_name)), "mmpose_keypoints2d", f"{index:04d}.pkl")   
                    
                            else: 
                                
                                img_name.replace("jpg", "pkl")

                            try:    
                                # bbox = joblib.load(pickle_name)[:4]
                                bbox = self.raw_bboxes[5*idx:5*idx+4][:4]
               
                            except:
                                print("Exception in loading bbox", bbox)
                    ...
                    if part == 'rhand':
                        img_part, _, crop_shape = self.rgb_processing(img_orig, center_part, scale_part, [constants.IMG_RES, constants.IMG_RES], flip=0)
                        item[f'img_rhand_unnorm'] = torch.from_numpy(img_part).float()
                    else:
                        img_part = img_orig.copy()[:constants.IMG_RES, :constants.IMG_RES, :].T
                    
                    # Ignore face and left hand for now. We dont care about their bounding boxes
                    # if part in ['lhand', 'face']:
                    #     img_part = img_orig.copy()[:constants.IMG_RES, :constants.IMG_RES, :].T
                    # else:
                    #     img_part, _, crop_shape = self.rgb_processing(img_orig, center_part, scale_part, [constants.IMG_RES, constants.IMG_RES], flip=0)
